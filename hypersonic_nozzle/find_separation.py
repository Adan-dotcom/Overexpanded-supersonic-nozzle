...
def wall_analysis() -> tuple[list[dict[str, float]], list[float], list[float]]:
    ...
    divergent = [row for row in rows if row["x"] >= 0.0]
...
def centerline_analysis() -> tuple[list[dict[str, float]], dict[str, float]]:
    ...
    divergent = [row for row in centerline if row["x"] >= 0.0]
